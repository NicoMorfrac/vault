...
# Minimum Fusion Execution Record

For each actual controlled Fusion execution, retain or report as applicable:

- Paperclip issue / CAD ID;
- run revision;
- bridge version;
- Fusion application/build reported by the live capability;
- selected declarative operation;
- source instruction/reference;
- source attachment ID where applicable;
- expected source SHA-256 where applicable;
- geometry parameters;
- units;
- explicit assumptions;
- requested new output basename;
- queue/job identity;
- receipt identity;
- execution state;
- generated output files;
- output SHA-256 hashes;
- body/feature/parameter counts where returned;
- warnings/errors;
- verification result.

Where the bridge uses a disposable or scratch Fusion document, its lifecycle must remain controlled by the bridge. Do not leave an unintended scratch model as an authoritative or active design master.

A run record documents what the controlled bridge actually executed.

It does not establish:

- engineering approval;
- geometric correctness beyond the verified inputs;
- manufacturing release;
- external release.
