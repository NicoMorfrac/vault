...
For a direct local-board CAD/Fusion/2D/3D request, use `route_cad_task` after `read_task` and `checkout_task`. It accepts a fully dimensioned written instruction without an attachment and preserves supported sketch, drawing, PDF/image, DXF/SVG and 3D CAD attachments on the same issue. The connector verifies human origin, explicit CAD intent and the exact idle Drafting recipient. This route does not create a project or approve manufacture, external release or overwrite.
