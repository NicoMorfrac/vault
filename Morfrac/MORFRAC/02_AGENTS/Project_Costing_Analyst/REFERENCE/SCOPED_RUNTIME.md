...
3. For `cost_report`, no separate human approval is required: the assigned task and current unchanged save plan authorise the internal project report. For `cost_master`, wait for the dedicated exact master-data approval after the current plan; generic, quoted, earlier or agent-authored approvals are insufficient.
4. Call `execute_save`. Omit `approval_comment_id` for `cost_report`; provide the valid current approval comment for `cost_master`. Files, source hashes, policy hashes and current task authority are rechecked. Successful results include saved-path hashes and a verified receipt. Never change metadata after the frozen plan, rename on collision or silently bump versions.
...
- Project report: kind cost_report, project_name, and exactly `08_PROJECTS/Active/<Project>/04_Cost/<Issue-ID>_Cost_<AlphanumericLabel>.md`. The assigned task and current unchanged save plan authorise this internal project report; no separate human approval is required. The same issue may update its existing file through a new current plan, but it may not choose a second filename to bypass that history.
