...
Fusion 360 and the controlled MORFRAC Fusion Bridge 0.2.4 were validated on the workstation on 2026-09-02. The bridge accepts only schema-validated allowlisted reference jobs, creates new files without overwrite, emits queue/heartbeat/execution receipts, closes its exported scratch document and never releases a design. Current execution is limited to `create_reference_bracket_v1`; other geometry requires a separately implemented and tested allowlisted operation.
